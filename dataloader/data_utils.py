...
def get_base_dataloader(args):
    ...
    if args.dataset == 'classroom':
        ...
        trainset = args.Dataset.ClassRoom(train=True,classroom_type = args.train_mode,session = 0,
                                          crop_transform=crop_transform, secondary_transform=secondary_transform)
        testset = args.Dataset.ClassRoom(train=False,classroom_type = args.train_mode,session = 0
                                         )
...
def get_new_dataloader(args,session):
    ...
    if args.dataset == 'cifar100':
        ...
        trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=False, index=class_index, base_sess=False,
                                        crop_transform=crop_transform, secondary_transform=secondary_transform)
    if args.dataset == 'classroom':
        trainset = args.Dataset.ClassRoom(train=True,classroom_type = 'resnet_a1',session = session,
                                          crop_transform=crop_transform, secondary_transform=secondary_transform)
    if args.dataset == 'cub200':
    ...
    if args.dataset == 'mini_imagenet':
        testset = args.Dataset.MiniImageNet(root=args.dataroot, train=False,
                                      index=class_new)
    if args.dataset == 'classroom':
        testset = args.Dataset.ClassRoom(train=False,classroom_type = 'resnet_a1',session = session)
