...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None, trans=1):
        ...
        if self.args.dataset in ['classroom']:
            self.encoder_q = resnet18(False,args,num_classes=self.args.moco_dim)#定义对比学习特征表示的维度
            self.encoder_k = resnet18(False,args,num_classes=self.args.moco_dim)
            self.num_features = 512
        if self.args.dataset in ['cifar100']:
            self.encoder_q = resnet20(num_classes=self.args.moco_dim)#定义对比学习特征表示的维度
        ...
        #moco设置    
        self.K = self.args.moco_k#队列长度
        self.m = self.args.moco_m#动量改变
        self.T = self.args.moco_t#logits温度
        ...
        self.queue = nn.functional.normalize(self.queue, dim=0)#包含了负样本特征向量的队列
    ...
    def forward(self, im_cla, im_q=None, im_k=None, labels=None, im_q_small=None, base_sess=True, 
        ...
        if self.mode != 'encoder':
            if im_q == None:
                ...
                targets = ((labels[:, None] == self.label_queue[None, :]) & (labels[:, None] != -1)).float().cuda()#在队列里面筛选出来负样本
