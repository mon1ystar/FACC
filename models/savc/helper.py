...
def base_train(model, trainloader, criterion, optimizer, scheduler, epoch, transform, args):
    tl = Averager()
    tl_joint = Averager()#联合损失
    tl_moco = Averager()#对比损失
    tl_moco_global = Averager()#对比全局损失
    tl_moco_small = Averager()
    ta = Averager()
    model = model.train()
    tqdm_gen = tqdm(trainloader)
    for i, batch in enumerate(tqdm_gen, 1):
        data, single_labels = [_ for _ in batch] #[原本，两大裁切，四小裁切] 正常标签
        b, c, h, w = data[1].shape
        original = data[0].cuda(non_blocking=True)#原本图像，异步提高数据加载效率
        ...
        #把四组小尺寸裁剪的图像拼接到了一起 4x[128,3,18,18] -> [512,3,18,18]
        if len(args.num_crops) > 1:
            data_small = data[args.num_crops[0]+1].unsqueeze(1)#增加一个维度
        ...
        data_classify = transform(original)   # 旋转180+本身[128,,,]->[256,,,]
        data_query = transform(data[1])
        data_key = transform(data[2])
        data_small = transform(data_small)
        m = data_query.size()[0] // b #M=2
        joint_labels = torch.stack([single_labels*m+ii for ii in range(m)], 1).view(-1) #扩展标签（对应分类器也要扩展）分类器的数量*fansty_num
